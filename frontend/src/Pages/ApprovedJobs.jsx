    ...
    useEffect(() => {
        ...
        const fetchJobs = async () => {
          try {
            ...
            const {data} = await axios.get(`http://localhost:4005/api/v1/employer/approved-job-by-employer?page=${page}&${limit}`,{withCredentials:true,
                headers: {
                    Authorization: `Bearer ${token}` // Ensure it's prefixed with "Bearer"
                },});
                setJobs(data.approved_jobs);
        ...
        if (window.confirm("Are you sure you want to delete this job?")) {
          try {
            ...
            if(data.success){
                ...
                setJobs((prevJob) => prevJob.filter((jobs) => jobs._id !== item._id));
            }else{
  ...
  return (
   <Container fluid>
    <Row>
        ...
        <h2 className='text-center pb-3'>Approved Jobs</h2>
      ...
      <tbody>
       ...
       jobs.length > 0 ? (jobs && jobs.map((item,i)=>(
        <tr key={i}>
          ...
          <FaTrash  title='Delete' style={{color:'red',cursor:'pointer'}} onClick={(event)=>handleDelete(item,event)}/> 
          </td>
        </tr>
        ))):( <tr className='text-center' >
          <td colSpan={8}>No approved Jobs available..!</td>
          </tr> )}
