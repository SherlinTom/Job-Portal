    ...
    useEffect(() => {
        ...
        const fetchJobs = async () => {
          try {
            ...
            const {data} = await axios.get(`http://localhost:4005/api/v1/employer/rejected-job-by-employer?page=${page}&${limit}`,{withCredentials:true,
                headers: {
                    Authorization: `Bearer ${token}` // Ensure it's prefixed with "Bearer"
                },});
                setJobs(data.rejected_jobs);
  ...
  return (
   <Container fluid>
    <Row>
        ...
        <h2 className='text-center pb-3'>Rejected Jobs</h2>
      ...
      <tbody>
       ...
       jobs.length > 0 ? (jobs && jobs.map((item,i)=>(
        <tr key={i}>
          ...
          <td colSpan={8}>No Rejected Jobs available..!</td>
          </tr> )}
