    ...
    useEffect(() => {
        ...
        const fetchJobs = async () => {
          try {
            ...
            const {data} = await axios.get(`http://localhost:4005/api/v1/employer/pending-job-by-employer?page=${page}&${limit}`,{withCredentials:true,
                headers: {
  ...
  return (
   <Container fluid>
    <Row>
        ...
        <h2 className='text-center pb-3'>Pending Jobs</h2>
      ...
      <tbody>
       ...
       jobs.length > 0 ? (jobs && jobs.map((item,i)=>(
        <tr key={i}>
          ...
          <td colSpan={8}>No Pending Jobs available..!</td>
          </tr> )}
